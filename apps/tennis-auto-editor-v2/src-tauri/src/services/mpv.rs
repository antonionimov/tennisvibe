...
mod imp {
    ...
    impl MpvController {
        pub fn ensure_player(&mut self, _media_path: &Path) -> Result<String, String> {
            Err("当前平台暂不支持内置 mpv 标注播放器".to_string())
        }
        ...
        pub fn seek_relative(&mut self, _delta_sec: f64) -> Result<(), String> {
            Err("当前平台暂不支持内置 mpv 标注播放器".to_string())
        }

        pub fn seek_absolute(&mut self, _time_sec: f64) -> Result<(), String> {
            Err("当前平台暂不支持内置 mpv 标注播放器".to_string())
        }
